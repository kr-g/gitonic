...
def check_and_bring_to_front(sock, wndw, accept_and_close=True):
    ...
    print_t("received instance call")
...
def switch2instance(path):
    ...
    _sock, port = check_instance(_fnam)

    if _sock is None:
        create_client_socket(port)

    return _sock is None


def tk_bg_bring_to_front(root):
    global _sock

    check_and_bring_to_front(_sock, root)
    root.after(500, lambda: tk_bg_bring_to_front(root))
